...
struct demux_stream {
    ...
    double last_key_ts;     // timestamp of last keyframe packet
    size_t last_key_bytes;  // summed packet sizes since last keyframe packet
    double bitrate;
...
static void ds_flush(struct demux_stream *ds)
{
    ...
    ds->packs = 0;
    ds->bytes = 0;
    ds->last_ts = ds->base_ts = ds->last_key_ts = MP_NOPTS_VALUE;
    ds->last_key_bytes = 0;
    ds->bitrate = -1;
    ds->eof = false;
    ds->active = false;
...
static struct demux_packet *dequeue_packet(struct demux_stream *ds)
{
    ...
    if (pkt->keyframe) {
        // Update bitrate
        if (ts == MP_NOPTS_VALUE || ds->last_key_ts == MP_NOPTS_VALUE ||
            ts == ds->last_key_ts)
        {
            ds->bitrate = -1;
        } else {
            ds->bitrate = ds->last_key_bytes / (ts - ds->last_key_ts);
        }
        ds->last_key_ts = ts;
        ds->last_key_bytes = 0;
    }
    ds->last_key_bytes += pkt->len;
